...
let
  ...
  networkName = "3d-printer";
in
{
  # Server for low-level comms with 3D printer Klipper firmware
  services.klipper = {
    enable = true;
  ...
  # Server for interacting with Klipper
  services.moonraker = {
    user = "root";
    enable = true;
    address = "0.0.0.0";
  ...
  services.fluidd = {
    ...
    nginx.locations."/webcam".proxyPass = "http://127.0.0.1:8081/stream";
  };
